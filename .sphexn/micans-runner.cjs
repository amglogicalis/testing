...
async function run() {
  ...
  // Automatic GitHub Synchronization in sync mode when run in CI
  if (mode === 'sync' && githubToken && targetRepo && targetRepo.includes('/')) {
    ...
    if (totalPatches > 0) {
      if (shouldCreatePr) {
        console.log(`[MODO SYNC-PR] Preparando Pull Request de sincronización en ${targetRepo}...`);
        ...
        try {
          ...
          if (prRes.status === 201 && prRes.body && prRes.body.html_url) {
            ...
            console.log(`Aviso en creación de PR (Status ${prRes.status}): ${prRes.body && prRes.body.message ? prRes.body.message : ''}`);
          }
        } catch (gitErr) {
